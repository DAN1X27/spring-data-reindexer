 ...
 */
final class ReindexerQueryBlocks {
	...
	static DerivedExecutionCodeBlockBuilder derivedQueryExecutionCodeBlockBuilder(PartTree tree,
			AotQueryMethodGenerationContext context, ReindexerMappingContext mappingContext,
	...
	static final class DerivedQueryCodeBlockBuilder {
		...
		CodeBlock build() {
			...
			builder.add("$1T<$2T> $3L = query($2T.class)", Query.class, context.getDomainType(),
			...
			builder.add(createJoinCodeBlocks(entity));
		...
		private CodeBlock createJoinCodeBlocks(ReindexerPersistentEntity<?> entity) {
			CodeBlock.Builder builder = CodeBlock.builder();
		...
		private CodeBlock createWhereCodeBlock(Part part, Iterator<String> allParameterNames) {
			...
			return switch (part.getType()) {
				...
				case NEGATING_SIMPLE_PROPERTY, SIMPLE_PROPERTY -> {
					...
					builder.add(CodeBlock.of(".like($1S, $2L)", indexName, allParameterNames.next()));
					yield builder.build();
				}
	...
	static final class DerivedExecutionCodeBlockBuilder {
		...
		CodeBlock build() {
			...
			if (this.tree.isDelete()) {
				Assert.isTrue(this.context.getMethodReturn().isVoid(),
						"The return type of derived delete method must be void");
			...
			if (this.queryMethod.isSliceQuery()) {
				Assert.notNull(pageableParameterName,
						"The Pageable parameter name must not be null for Slice derived query method");
				return builder
		...
		private CodeBlock createDynamicProjectionCodeBlock(String root, String dynamicProjectionRef) {
			CodeBlock.Builder builder = CodeBlock.builder();
			...
			builder.addStatement("$1T $2L = $1T.of($3L, $4T.class, getProjectionFactory())", ReturnedType.class,
					returnedType, dynamicProjectionRef, this.context.getDomainType());
		...
		private CodeBlock createSortCodeBlock(String root, String sortParameterRef) {
			CodeBlock.Builder builder = CodeBlock.builder();
			String order = this.context.localVariable("order");
			builder.beginControlFlow("if ($L.isSorted())", sortParameterRef);
			builder.beginControlFlow("for ($1T $2L: $3L)", Sort.Order.class, order, sortParameterRef);
		...
		private CodeBlock createPageableCodeBlock(String root, String pageableParameterRef, Integer maxResults) {
			CodeBlock.Builder builder = CodeBlock.builder();
			builder.beginControlFlow("if ($L.isPaged())", pageableParameterRef);
			if (maxResults != null) {
				...
				builder.addStatement("int $1L = $2T.getOffsetAsInteger($3L)", firstResult, PageableUtils.class,
						pageableParameterRef);
				builder.addStatement(
						"$1L.offset($2L.getPageSize() > $3L && $4L > 0 ? $4L - ($2L.getPageSize() - $3L) : $4L)", root,
						pageableParameterRef, maxResults, firstResult);
			}
			else {
				...
				builder.addStatement("$1L.limit(%s).offset($3T.getOffsetAsInteger($2L))".formatted(limitExpr), root,
						pageableParameterRef, PageableUtils.class);
			}
			builder.endControlFlow();
			return builder.build();
		}
